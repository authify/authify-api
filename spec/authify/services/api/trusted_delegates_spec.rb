...
describe Authify::API::Services::API do
  ...
  # /trusted-delegates
  context 'trusted-delegates endpoints' do
    let(:new_delegate_data) do
      {
        'data' => {
          'type' => 'trusted-delegates',
          'attributes' => {
            'name' => 'test'
          }
        }
    ...
    context 'OPTIONS /trusted-delegates' do
      it 'returns 200 with expected headers' do
        options '/trusted-delegates'
    ...
    context 'GET /trusted-delegates' do
      it 'requires authentication' do
        header 'Accept', 'application/vnd.api+json'
        get '/trusted-delegates'
      ...
      it 'prevents non-admins from listing Trusted Delegates' do
        header 'Accept', 'application/vnd.api+json'
      ...
      it 'allows admins to list all Trusted Delegates' do
        header 'Accept', 'application/vnd.api+json'
        ...
        expect(details['data'].first['links']).to eq('self' => '/trusted-delegates/1')
        expect(details['data'].first).not_to have_key('relationships')
        expect(details).to have_key('jsonapi')
        expect(details['jsonapi']).to eq('version' => '1.0')
      end
    end

    context 'POST /trusted-delegates' do
      it 'requires authentication' do
        header 'Accept', 'application/vnd.api+json'
        header 'Content-Type', 'application/vnd.api+json'

        post '/trusted-delegates', new_delegate_data.to_json
      ...
      it 'prevents non-admins from adding a Trusted Delegate' do
        header 'Accept', 'application/vnd.api+json'
        header 'Content-Type', 'application/vnd.api+json'
        header 'Authorization', "Bearer #{user_jwt}"
      ...
      it 'prevents invalid users from adding a Trusted Delegate' do
        header 'Accept', 'application/vnd.api+json'
        header 'Content-Type', 'application/vnd.api+json'
        header 'Authorization', "Bearer a1#{bad_user_jwt}"
      ...
      it 'allows admins to add a Trusted Delegate' do
        header 'Accept', 'application/vnd.api+json'
        header 'Content-Type', 'application/vnd.api+json'
        header 'Authorization', "Bearer #{admin_jwt}"
        ...
        expect(details['data']['type']).to eq('trusted-delegates')
        ...
        expect(details['data']['links']).to eq('self' => '/trusted-delegates/2')
        expect(details['data']).not_to have_key('relationships')
        expect(details).to have_key('jsonapi')
        expect(details['jsonapi']).to eq('version' => '1.0')
      end
    end

    context 'DELETE /trusted-delegates/:id' do
      it 'requires authentication' do
        header 'Accept', 'application/vnd.api+json'
        header 'Content-Type', 'application/vnd.api+json'

        delete '/trusted-delegates/2'
      ...
      it 'prevents non-admins from deleting Trusted Delegates' do
        header 'Accept', 'application/vnd.api+json'
        header 'Authorization', "Bearer #{user_jwt}"
      ...
      it 'allows admins to remove an API key' do
        header 'Accept', 'application/vnd.api+json'
        header 'Content-Type', 'application/vnd.api+json'
        header 'Authorization', "Bearer #{admin_jwt}"
